...
$hora_desde=date("H:i:s", time()-2400 );
$hora_hasta=date("H:i:s");
...
//$query='select distinct numero_venta from ventas where sucursal="'.$sucursal.'" and fecha="'.$fecha.'" and hora >= "'.$hora_hasta.'" order by fecha, hora desc limit 0,20';
...
while($row=mysql_fetch_array($result)){
	...
	if(($array["tipo_pago"]=="ta" OR $array["tipo_pago"]=="de") AND  $array["tiquet"]==""  ){
		echo '<td><A HREF="tiquet_agrega.php?id_sucursal='.$id_sucursal.'&numero_venta='.$row["numero_venta"].'"><button>N TIQUET</button></A></td>';
		echo '<td>NO AUTORIZADA</td>';
	}else{
		echo '<td></td>';
...
/*
$query='select distinct numero_venta from ventas where sucursal="'.$sucursal.'" and
                                                        fecha>="2020-11-01" and 
                                                        tipo_pago!="co" and
                                                         (tiquet is NULL or tiquet="") 
                                                                order by fecha, hora desc';
